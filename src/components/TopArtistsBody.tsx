...
const TopArtistsLongTerm = (props: Props): JSX.Element => {
	...
	useEffect(() => {
		if (!api) return
		if (artists[props.term]) return

		const half_artists: SpotifyApi.ArtistObjectFull[] = []

		api.getMyTopArtists({ limit: 50, time_range: props.term })
			.then(res => {
				half_artists.push(...res.items)
				return api.getMyTopArtists({ offset: 49, limit: 50, time_range: props.term })
			})
			.then(res => {
				dispatch(props.action([...half_artists, ...res.items.slice(1)]))
			})
			.catch(err => {
				dispatch(set_error(err))
			})
	}, [dispatch, api, artists, props])
	...
	return (
		<Container>
			...
			<Grid sx={{ my: 3 }} container spacing={5} justifyContent="space-evenly">
				{artists[props.term]?.map((artist, i) => (
