...
import { set_statistics_tracks_short_term } from "../actions/StatisticsActions"
import useSpotifyApi from "../hooks/useSpotifyApi"

const TopTracksShortTerm = (): JSX.Element => {
	const api = useSpotifyApi()
	const dispatch = useDispatch()
	const tracks = useSelector(state => state.statistics.tracks.short_term)
	...
	useEffect(() => {
		...
		api.getMyTopTracks({ limit: 50, time_range: "short_term" })
			.then(res => {
				...
				return api.getMyTopTracks({ offset: 49, limit: 50, time_range: "short_term" })
			})
			.then(res => {
				dispatch(set_statistics_tracks_short_term([...half_tracks, ...res.items]))
			})
			.catch(err => {
