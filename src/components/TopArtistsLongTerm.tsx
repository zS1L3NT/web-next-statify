...
import { set_statistics_artists_long_term } from "../actions/StatisticsActions"
...
const TopArtistsLongTerm = (): JSX.Element => {
	...
	const artists = useSelector(state => state.statistics.artists.long_term)
	...
	useEffect(() => {
		...
		api.getMyTopArtists({ limit: 50, time_range: "long_term" })
			.then(res => {
				...
				return api.getMyTopArtists({ offset: 49, limit: 50, time_range: "long_term" })
			})
			.then(res => {
				dispatch(set_statistics_artists_long_term([...half_artists, ...res.items]))
			})
			.catch(err => {
